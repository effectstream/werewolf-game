/**
 * Deploy Werewolf Contract to Midnight mainnet network.
 *
 * Usage:
 *   deno task midnight-contract:deploy:mainnet           # deploy contract
 *   deno task midnight-contract:deploy:mainnet --wallet  # print wallet addresses and exit
 *
 * This script:
 *   1. Loads .env.mainnet from the repo root
 *   2. Spawns the Midnight proof server configured for mainnet (MainNet ledger)
 *   3. Waits for the proof server to be ready on :6300
 *   4. Deploys the Werewolf contract to the mainnet network
 *
 * Dust generation:
...
const ENV_PATH = join(SCRIPT_DIR, "../../../../.env.mainnet");
...
const NETWORK_ID = "mainnet" as NetworkId.NetworkId;

// URLs: prefer explicit overrides from .env.mainnet, then auto-derive from network ID
const MAINNET_URLS: NetworkUrls = {
  id: NETWORK_ID,
...
function startProofServer(): Deno.ChildProcess {
  console.log(
    "[proof-server] Starting Midnight proof server for mainnet (MainNet ledger)...",
  );
...
async function printWalletAddresses(seed: string): Promise<void> {
  ...
  // Encodes coin public key + encryption public key into the mn_addr_mainnet1… Bech32m format,
  ...
  const walletResult = await buildDeployWalletFacade(
    {
      indexer: MAINNET_URLS.indexer!,
      indexerWS: MAINNET_URLS.indexerWS!,
      node: MAINNET_URLS.node!,
      proofServer: MAINNET_URLS.proofServer!,
    },
    seed,
    NETWORK_ID,
  );
...
const seed = MAINNET_URLS.walletSeed ??
...
if (!seed) {
  console.error(
    "ERROR: MIDNIGHT_WALLET_SEED is not set. Add it to .env.mainnet.",
  );
  Deno.exit(1);
}
