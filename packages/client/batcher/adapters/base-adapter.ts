...
import { Transaction as LegacyLedgerTransaction } from "@midnight-ntwrk/ledger-v7";
...
type BindingMarker = "pre-binding" | "binding";
 ...
 */
export class MidnightBalancingAdapterX
  ...
  private provingProvider: ReturnType<typeof httpClientProvingProvider> | null =
    null;
  ...
  private async initialize(walletSeed: string): Promise<void> {
    try {
      ...
      if (this.config.zkConfigPath) {
        ...
        this.provingProvider = httpClientProvingProvider(
      ...
      console.error(
        "❌ Failed to initialize Midnight Balancing Adapter:",
        error,
      );
      throw error;
    }
  ...
  private async ensureFunds(): Promise<void> {
    ...
    try {
      ...
      if (dustBalance === 0n) {
        console.warn(
          "⚠️ Warning: Filler wallet has 0 dust balance. Submissions may fail.",
        );
      }
    } catch (error) {
  ...
  private parseHexInput(
    ...
    if (trimmed.startsWith("{")) {
      ...
      const cleanHex = parsed.tx.startsWith("0x")
        ? parsed.tx.slice(2)
        : parsed.tx;
      return {
        hex: cleanHex,
  ...
  buildBatchData(
    ...
    try {
      const { hex: cleanHex, circuitId, txStage } = this.parseHexInput(
        input.input,
      );
      this.currentCircuitId = circuitId ?? this.config.circuitId ?? null;
      console.log(
        `🧾 [balancing] Received tx hex length=${cleanHex.length} target=${input.target} stage=${
          txStage ?? "auto"
        } circuitId=${this.currentCircuitId ?? "none"}`,
      );
      ...
      if (txStage === "unbound") {
        delegatedTx = LedgerTransaction.deserialize(
          "signature" as const,
          "proof" as const,
      ...
      try {
        const roundTripHex = Buffer.from(delegatedTx.serialize()).toString(
          "hex",
        );
        console.log(
        ...
        console.warn(
          "⚠️ [balancing] Failed to round-trip serialize tx:",
          error,
        );
      }
  ...
  async submitBatch(
    ...
    try {
      if (txStage === "unbound") {
        balancedRecipe = await this.walletResult.wallet
          .balanceUnboundTransaction(
            ...
            {
              ttl: createTtl(),
              // Delegated Werewolf calls arrive as already-proved unbound txs.
              // Only dust fee balancing is needed here; adding shielded balancing
              // introduces another tx instance from a different ledger boundary.
      ...
      try {
        ...
        console.error(
          "⚠️ [balancing] Failed to serialize delegated tx:",
          serError,
        );
      }
      throw error;
    ...
    try {
      ...
      console.warn(
        "⚠️ Failed to derive transaction hash from finalized tx:",
        error,
      );
    }
  ...
  private async finalizeWithProver(
    ...
    switch (recipe.type) {
      case "UNPROVEN_TRANSACTION": {
        const proven = await this.proveWithLegacyLedger(
          recipe.transaction,
          "pre-binding",
          zkConfig,
        );
      ...
      case "FINALIZED_TRANSACTION": {
        ...
        return recipe.originalTransaction.merge(
          proven.bind(),
        ) as FinalizedTransaction;
      }
    }
  ...
  private async proveWithLegacyLedger(
    tx: UnprovenTransaction,
    bindingMarker: BindingMarker,
    _zkConfig: unknown,
  ) {
    // Call tx.prove directly with our own CostModel and provingProvider to
    // avoid cross-WASM-module _assertClass failures caused by the proof
    // provider's transitive ledger-v7 version differing from the batcher's.
    const costModel = CostModel.initialCostModel();
    const proven = await tx.prove(this.provingProvider as any, costModel);
      ...
      [
        {
          ...
          outputs: [{
            ...
            receiverAddress:
              "mn_shield-addr_undeployed1jy8cy2attgg3vmtpyfsz0xfvf9zl9zcf70je90jl3ual67hcuy898ge625crq5vvz6sg0f594szy8ll9r8rfdg8zkxzlex9pdwt7aqcsme28p",
            amount: 1n,
          }],
        },
      ],
