 ...
 * wallet identity. The transaction is proved in-browser, then intercepted at
 * balanceTx and sent to the batcher for fee balancing and on-chain submission.
 *
 * Flow:
 ...
 *   5. Browser prover generates the unbound tx locally
 *   6. BatcherClient intercepts balanceTx, serialises the unbound tx, POSTs to batcher
 *   7. STF detects the on-chain vote via roundVotes ledger map → triggers resolution
 */
  ...
  if (!_compiledContract) {
    _compiledContract = (CompiledContract.withCompiledFileAssets as any)(
      (CompiledContract.withWitnesses as any)(
        (CompiledContract.make as any)("contract-werewolf", WerewolfContract),
        witnesses,
      ),
      window.location.origin,
    );
 ...
 * into an unbound transaction (proof generated by the browser proof provider
 * before balanceTx), intercepted at balanceTx, and forwarded to the batcher for
 * dust balancing and on-chain submission.
 *
...
export async function submitVoteOnChain(
   ...
   * This ensures the batcher always handles fee payment, even though proof
   * generation now happens locally in the browser.
   *
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (phase === "NIGHT" || phase === "night") {
        await batcherClient.nightAction(BigInt(gameId), callbacks?.onProofDone);
      } else {
        await batcherClient.voteDay(BigInt(gameId), callbacks?.onProofDone);
      }
